...
def calculate_Thunderwolves_stuff_plus_for_pitch_type(df, pitch_type, player_fastball_velocity=None):
    """Calculate Thunderwolves Stuff+ for a specific pitch type using updated weights"""
...
def calculate_player_stuff_plus(pitch_data):
    ...
    for pitch_category, pitch_variants in pitch_type_mapping.items():
        ...
        if len(pitch_type_data) >= 1:  # Lowered threshold to 1 pitch to show all pitch types
            stuff_plus = calculate_Thunderwolves_stuff_plus_for_pitch_type(
...
def create_stuff_plus_radar_chart(pitch_stuff, player_name):
    ...
    ax.fill(angles, stuff_values, alpha=0.3, color="
#0066B3")

    # Add reference line at 50th percentile (league average)
    ax.plot(angles, avg_line, linewidth=2, label='Team Average (100)', 
           linestyle='--', color='
#FFC72C', alpha=0.8)
...
def create_stuff_plus_bar_chart(pitch_stuff, player_name):
    ...
    bars = ax.bar(pitch_types, stuff_values, color="
#0066B3", alpha=0.8, edgecolor='white')
    ...
    ax.axhline(y=100, color='gray', linestyle='--', alpha=0.7, label='Team Average (100)')
...
def create_cmj_quadrant_analysis(exercise_data, player_name):
    ...
    NCC_decel_df = pd.DataFrame({'date': eccentric_decel_data['dates'], 'eccentric_decel': eccentric_decel_data['values']})
    NCC_brake_df = pd.DataFrame({'date': eccentric_brake_data['dates'], 'eccentric_brake': eccentric_brake_data['values']})

    # Merge all three metrics on date
    combined_df = pd.merge(conc_df, NCC_decel_df, on='date', how='inner')
    combined_df = pd.merge(combined_df, NCC_brake_df, on='date', how='inner')
        ...
        if exercise_code == 'HJ':
            ...
            if not metric_specific_data.empty:
                ...
                ax.scatter(repeats, values, 
                          s=150, color='
#54342c', edgecolors='white', linewidth=3, alpha=0.8)

                # Connect points with line
                ax.plot(repeats, values, color='
#54342c', linewidth=2, alpha=0.7)
            ...
            if len(daily_data) >= 2:
                ax.plot(daily_data['test_date'], daily_data['value'], 
                       marker='o', linewidth=3, markersize=10, color='
#54342c', 
                       markerfacecolor='
#54342c', markeredgecolor='white', markeredgewidth=2)
                ...
                if len(daily_data) >= 3:
                    ...
                    ax.text(0.02, 0.98, trend_text, transform=ax.transAxes, 
                            verticalalignment='top', bbox=dict(boxstyle='round,pad=0.5', facecolor='
#54342c', alpha=0.8, edgecolor='white'),
                ...
                ax.scatter([test_date], [value], 
                          s=200, color='
#54342c', edgecolors='white', linewidth=3, zorder=5)
...
#1e1e1e')
        ...
        if metric_names:
            ...
            bars1 = ax.bar(x - width/2, left_values, width, label='Left', color='
#4A90A4', alpha=0.8)
            bars2 = ax.bar(x + width/2, right_values, width, label='Right', color='
#C41E3A', alpha=0.8)
            ...
            ax.legend(facecolor='
#1e1e1e', edgecolor='white', labelcolor='white')
...
#C41E3A'], 
    ...
    if team_median_er is not None:
        legend = ax1.legend(loc='upper right', facecolor='
#1e1e1e', edgecolor='white', labelcolor='white')
...
#1e1e1e')
    ...
    ax2.barh(['This Player'], [ratio], height=0.3, color='
#C41E3A', alpha=0.9, edgecolor='white', linewidth=2)

    # Add reference lines from research
    ax2.axvline(x=0.70, color='
#FFD700', linestyle='--', linewidth=2, alpha=0.8)
    ax2.axvline(x=0.83, color='
#00CED1', linestyle='--', linewidth=2, alpha=0.8)
    ax2.axvline(x=1.0, color='white', linestyle=':', linewidth=2, alpha=0.5)

    # Add team median line if available
    if team_median_ratio is not None:
        ax2.axvline(x=team_median_ratio, color='
#32CD32', linestyle='-', linewidth=3, alpha=0.9)
        ax2.text(team_median_ratio, 0.55, f'Team\nMedian\n({team_median_ratio:.2f})', ha='center', va='bottom', 
                color='
#32CD32', fontsize=10, fontweight='bold')

    # Add reference labels at top
    ax2.text(0.70, 0.55, 'Injury Risk\nThreshold\n(0.70)', ha='center', va='bottom', 
            color='
#FFD700', fontsize=9, fontweight='bold')
    ax2.text(0.83, 0.55, 'Pro Pitcher\nAvg\n(0.83)', ha='center', va='bottom', 
            color='
#00CED1', fontsize=9, fontweight='bold')
...
#1e1e1e')
    ...
    bars = ax.bar(region_counts['Body Region'], region_counts['Test Count'], 
                  color='
#C41E3A', alpha=0.8, edgecolor='white')
...
def display_player_assessment_data(player_name):
    ...
    if not os.path.exists(excel_file_path):
        st.error("Assessment table file not found at data/ThunderwolvesBaseballTableAssessment.xlsx")
        return
...
#000000; border-radius: 5px; margin-bottom: 10px; color: white;'>
                <strong>Legend:</strong> 
                <span style='background-color: 
#cc0000; color: white; padding: 2px 8px; margin: 0 5px; border-radius: 3px; font-weight: bold;'>Mobility Deficiency (Must Improve)</span>
                <span style='background-color: 
#006400; color: white; padding: 2px 8px; margin: 0 5px; border-radius: 3px; font-weight: bold;'>Hyper Mobility (Must work to conserve) </span>
            </div>
            """, unsafe_allow_html=True)
...
#cc0000; color: white; padding: 10px; border-radius: 5px; font-weight: bold; text-align: center;'>
                                ...
                                st.markdown(f"""
                                <div style='background-color: 
#006400; color: white; padding: 10px; border-radius: 5px; font-weight: bold; text-align: center;'>
                                    <div style='font-size: 0.8em; margin-bottom: 5px;'>{column}</div>
                                    <div style='font-size: 1.5em;'>{value}</div>
...
#000000; border-radius: 5px; margin-bottom: 10px; color: white;'>
            <strong>Legend:</strong> 
            <span style='background-color: 
#cc0000; color: white; padding: 2px 8px; margin: 0 5px; border-radius: 3px; font-weight: bold;'>Needs Improvement</span>
            <span style='background-color: 
#006400; color: white; padding: 2px 8px; margin: 0 5px; border-radius: 3px; font-weight: bold;'>Positive Outlier (ensure maintenance)</span>
            <span style='margin-left: 15px; color: 
#B8D4E8;'>(All values in inches)</span>
        </div>
        """, unsafe_allow_html=True)
                ...
                with cols[col_idx % 3]:
                    if is_outlier:
                        if outlier_type == 'low':
                            st.markdown(f"""
                            <div style='background-color: 
#cc0000; color: white; padding: 10px; border-radius: 5px; font-weight: bold; text-align: center; margin-bottom: 10px;'>
                                <div style='font-size: 0.8em; margin-bottom: 5px;'>{column}</div>
                                <div style='font-size: 1.5em;'>{value}"</div>
                            ...
                            st.markdown(f"""
                            <div style='background-color: 
#006400; color: white; padding: 10px; border-radius: 5px; font-weight: bold; text-align: center; margin-bottom: 10px;'>
                                <div style='font-size: 0.8em; margin-bottom: 5px;'>{column}</div>
                                <div style='font-size: 1.5em;'>{value}"</div>
...
def main():
    # Header
    st.markdown('<h1 class="main-header">NCC Thunderwolves Baseball</h1>', unsafe_allow_html=True)
    ...
    if selected_player:
        ...
        st.markdown('<h3 class="section-header">Thunderwolves Stuff+ Overview</h3>', unsafe_allow_html=True)
        ...
        # Stuff+ Summary Table
        st.markdown('<h3 class="section-header">Thunderwolves Stuff+ Summary</h3>', unsafe_allow_html=True)
        ...
        for pitch_type in valid_pitch_types:
            ...
            if len(pitch_subset) > 0:
                ...
                if stuff_plus_val is None and len(velocity_clean) > 0:
                    ...
                    try:
                        stuff_plus_val = calculate_Thunderwolves_stuff_plus_for_pitch_type(
        ...
        # Add player horizontal jumps data
    ...
    # Footer
    st.markdown("---")
    st.markdown("*NCC Thunderwolves Player Lookup | Made by Liquid Sports Lab*")
